...
"""
    ...
    group = parser.add_argument_group("TeX converter content and representation options")
    ...
    group = parser.add_argument_group("TeX converter sort options")
    group.add_argument("--msg-sort", dest="msg_sort_key", choices=Converter.MSG_SORT_KEYS, default=Converter.MSG_SORT_KEY_ID, help="how to sort the messages in the pdf")
    group.add_argument("--sig-sort", dest="sig_sort_key", choices=Converter.SIG_SORT_KEYS, default=Converter.SIG_SORT_KEY_START_BIT, help="how to sort the signals in a message")

    group = parser.add_argument_group("TeX converter text options")
    group.add_argument("--title", help="the title of the document. The default is derived from outfile.")
    group.add_argument("--date", help="the date to be printed on the document. The default is today.")
    group.add_argument("--date-format", default="%d.%m.%Y", help="how to format the date. see https://docs.python.org/3/library/datetime.html#strftime-and-strptime-format-codes")
    group.add_argument("--header-left", default=r"", help="a text printed in the header left aligned")
    group.add_argument("--header-right", default=r"", help="a text printed in the header right aligned")
    group.add_argument("--footer-left", default=r"Created from \infile, \thedate", help="a text printed in the footer left aligned")
    group.add_argument("--footer-right", default=r"Page \thepage\ of \pageref{LastPage}", help="a text printed in the footer right aligned")
    ...
    group.add_argument("--details-header", default=r"%\subsection{Comments, allowed values and multiplexing details}", help=r"header of a list showing comments, values and multiplexing details for all signals where details are known")
    group.add_argument("--choices-header", default="Allowed values:", help="header of a list of the allowed values of the current signal in the details list (see also --before-choices-header)")

    group.add_argument("--mux-pattern-none", default="~", help="printed if a signal has no multiplexer_ids")
    group.add_argument("--mux-pattern-one", default=r"This signal is available only if {multiplexer_signal} has the value {multiplexer_id}.", help="printed if a signal has exactly one multiplexer_id")
    group.add_argument("--mux-pattern-many", default=r"This signal is available only if {multiplexer_signal} has one of the values {multiplexer_ids}.", help="printed if a signal has several multiplexer_ids")
    group.add_argument("--mux-id-pattern", default=r"0x{:x}", help="how to format a multiplexer id")
    group.add_argument("--mux-sep", default=r", ", help="a seperator inserted between several multiplexer ids")
    group.add_argument("--mux-sep-last", default=r" or ", help="a seperator inserted between the last and second last multiplexer ids")

    group = parser.add_argument_group("TeX converter layout options")
    group.add_argument("--document-class", default='article', help="the LaTeX document class to use")
    group.add_argument("--class-option", dest="class_options", default=['a4paper'], action="append", help="an option that you want to pass to the document class. Use this several times if you want to pass several class options.")
    ...
    group.add_argument("--before-details", default=r"\begin{description}", help=r"TeX code inserted before the details list but after the header (see --details-header)")
    group.add_argument("--detail-item-pattern", default=r"\item[{name}] ", help=r"the item label for the current signal in the details list, see --before-details")
    group.add_argument("--after-details", default=r"\end{description}", help=r"TeX code inserted after the details list, see --before-details")

    group.add_argument("--before-choices-header", default="\t\\begin{tabular}{@{}l@{ }l}\n\t\multicolumn{2}{@{}l}{", help="TeX code inserted before --choices-header")
    group.add_argument("--before-choices", default="} \\\\\n", help="TeX code inserted before the list of allowed values but after --choices-header")
    group.add_argument("--choice-pattern", default="\t"+r"\quad\textbullet~{num_value} & {str_value} \\", help="how an allowed value is formatted (see also --before-choices-header and --before-choices)")
    group.add_argument("--choice-sep", default="\n", help="TeX code inserted between several allowed values")
    group.add_argument("--choice-sep-last", default="\n", help="TeX code inserted between the last and second last allowed value")
    group.add_argument("--after-choices", default="\n\t\\end{tabular}", help="TeX code inserted after the list of allowed values")
