...
class PyplotMock:
    ...
    def __enter__(self):
        self._patches = [mock.patch(f'matplotlib.pyplot.{a}', getattr(self,a)) for a in self._attrs]
    ...
    def __init__(self, ignore=True, *, parent=None, **kw):
        ...
        for a in self._ATTRIBUTES_WHICH_CAN_BE_IGNORED:
            ...
            if kw.pop(key, ignore):
                setattr(self, a, mock.Mock(name=f'{a}-mock'))
                self.__kw[key] = True
            else:
                self.__kw[key] = False
    ...
    def __plot(self, xs, ys, fmt, *, label=None):
        ...
        if not self.__ignore_color:
            for i, m in enumerate(out):
                self.attach_mock(m, f'plot()._getitem_{i}')
...
"""
    ...
    def test_plot_rt(self):
        argv = ['cantools', 'plot', self.DBC_FILE]
        input_data = """\
(1609395080.446193) vcan0 00000343#B504CB04AE04BC04 R
(1609395081.447989) vcan0 00000343#650457045E047404 R
(1609395082.449807) vcan0 00000343#1504240415043204 T
(1609395083.451020) vcan0 00000343#DB03DB03DB03C503 T
(1609395084.452815) vcan0 00000343#6903850369037703 T
"""
...
"""
    ...
    def test_two_axes_with_auto_ylabels_and_legend_with_units(self):
        argv = ['cantools', 'plot', '--show-units', self.DBC_FILE, '*_33.*fl*:b', ',', '*_2.*fl*:r']
        input_data = """\
 (1612000388.291144)  vcan0  00000343   [8]  6B 04 87 04 8E 04 87 04
 (1612000388.291593)  vcan0  0000024A   [8]  7C 0A 75 0A 75 0A 8B 0A
 (1612000389.293447)  vcan0  00000343   [8]  F9 03 EB 03 00 04 F9 03
...
"""
        ...
        expected_subplot_calls = [
            [
                mock.call.plot(xs33, whlspeed_fl, 'b', label='BREMSE_33.whlspeed_FL [m/s]'),
                mock.call.set(ylabel='*_33.*fl*'),
                mock.call.set_xlabel(self.XLABEL_tA % self.parse_start_time(xs33[0])),
                mock.call.twinx(),
            ], [
                mock.call.plot(xs2, whlspeed_fl_bremse2, 'r', label='BREMSE_2.whlspeed_FL_Bremse2 [m/s]'),
                mock.call.set(ylabel='*_2.*fl*'),
                mock.call.set_xlabel(self.XLABEL_tA % self.parse_start_time(xs2[0])),
                mock.call.legend([None, None], ['BREMSE_33.whlspeed_FL [m/s]', 'BREMSE_2.whlspeed_FL_Bremse2 [m/s]']),
            ],
        ]
